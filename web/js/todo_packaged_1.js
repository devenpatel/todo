...
function updateTodayList() {
    ...
    jQuery.get("user/load_element.php",{
        date: currentDate
    },function(L, O){
        $("#tlist").html(L);
    });
...
$(document).ready(function(a){
    a("a.thickboxlink").click(function(){
        if (isDate(a("#datef").val()) && isDate(a("#datef").val())) {
            if(!a("#task").val()){
                alert('Please Enter Task.');
                return false;
            }
            var ss__href = this.href;
            ss__href += "?task=" + a("#task").val() + "&sdate=" + a("#datef").val() + "&edate=" + a("#datet").val();
